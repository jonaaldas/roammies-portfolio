...
export default defineEventHandler(async event => {
	...
	try {
		...
		const endpointSecret = 'whsec_gnsZBMvBIiCiPnm7TXS5DZ6WXOkOxp8u'; //dev ngrok
		...
		try {
			...
			if (stripeEvent.type == 'payment_intent.succeeded') {
				...
				await nitro.db.execute({
					sql: 'INSERT INTO customers (stripe_customer_id, stripe_subscription) VALUES (:stripe_customer_id, :stripe_subscription)',
					args: {
						stripe_customer_id: stripeEvent.data.object.customer,
						stripe_subscription: stripeEvent
					}
